...
# annotations through sys.modules[cls.__module__] -- None if unregistered,
# and the import dies on Python 3.12 with a bare
...
def _load_by_path(name: str, filename: str):
    ...
    sys.modules[spec.name] = mod  # see the note on the loader above
...
def run(input_path: str | Path, output_dir: str | Path = "./cat_outputs") -> dict:
    ...
    diagnostics, diagnostic_failures = diag.compute_all_21(catdata, target_level=200)
    for k in diagnostics:
        tag = "hand" if k in diag.HANDCODED_KEYS else "rojak"
        status = diagnostics[k].attrs.get("Q_INTEG_4_status")
        flag = "  [Q-INTEG-4: FAILED, all-NaN placeholder]" if status == "FAILED_ALL_NAN" else ""
        print(f"    [{tag:5s}] {k:22s} shape={tuple(diagnostics[k].shape)}{flag}")
    if diagnostic_failures:
        print(f"\n>>> Q-INTEG-4: {len(diagnostic_failures)} diagnostic(s) failed this chunk "
              f"and were replaced with all-NaN placeholders (excluded downstream via "
              f"Q-INTEG-3 skipna, not counted as non-exceedance):")
        for f in diagnostic_failures:
            print(f"      #{f.wj_number:>2} {f.key:<23} {f.exception_type}: {f.message}")
    # NOTE: diagnostic_failures is per-CHUNK (one input_path/one call to run()).
    # The 1979-2020 driver loops run() once per chunk (month/year) -- whoever
    # owns that outer loop should concatenate diagnostic_failures across all
    # calls into the actual run-level (42-year) summary; this function only
    # has visibility into its own single chunk.
...
def run_layers_2_to_6(
    ...
    exceedance_mean_ds = agg.exceedance_mean_all_severities(
        diagnostic_fields, thresholds, severity_names, signs
    )  # Q-CALIB-6 guard runs automatically inside this call
    # Q-INTEG-3 (Bug 1, option (b)): each value is now an xr.Dataset with
    # "exceedance_mean" (skipna mean over populated diagnostics) AND
    # "populated_count" (how many of the 21 actually contributed at that
    # cell/timestep) -- split them here rather than pass the Dataset
    # further down silently. `annual_agg.annual_exceedance_probability`
    # below is UNCHANGED and only sees the mean, same as before this fix.
    exceedance_mean: dict[str, xr.DataArray] = {
        sev: ds["exceedance_mean"] for sev, ds in exceedance_mean_ds.items()
    }
    populated_count: dict[str, xr.DataArray] = {
        sev: ds["populated_count"] for sev, ds in exceedance_mean_ds.items()
    }
    for sev_name, count_field in populated_count.items():
        n_diag = len(diagnostic_fields)
        min_count = int(count_field.min())
        if min_count < n_diag:
            print(f"    [Q-INTEG-3] {sev_name}: populated_count ranges "
                  f"{min_count}-{n_diag} of {n_diag} diagnostics -- some "
                  f"cells/timesteps were built from fewer than all 21 "
                  f"(gapped diagnostic(s) excluded via skipna, not "
                  f"counted as non-exceedance)")
    # NOTE: populated_count is NOT yet fed into Layer 5's annual
    # aggregation or Layer 6's trend fit -- whether/how
    # annual_exceedance_probability should weight or report on a
    # per-cell denominator that varies is a Q-AGG-5-adjacent design
    # decision, not decided here. It is kept alive (not discarded) so
    # that decision can be made deliberately later; it is not silently
    # dropped, but it is also not silently wired in.

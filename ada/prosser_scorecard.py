...
def read_per_diagnostic(outputs: Path, season: str, severity: str, tag: str = ""):
    """Read a per-diagnostic fit table.

    `tag` selects WHICH series. ada/per_diagnostic_trend.py writes
    `per_diagnostic_<season>_<severity>.csv` for the baseline and
    `..._audit.csv` for the audit re-derive (STATUS.md 18.13), so the two
    tables coexist and neither can overwrite the other. Default is the
    baseline, so every earlier invocation of this script is unchanged.

    WHY THE TAG EXISTS. On 2026-09-15 a run of jobs/21 without it wrote the
    audit-threshold numbers over the BASELINE csv, clobbering the table cited
    by RESULT_full_trend_42yr.md 4 and STATUS.md 15.6. Filenames are the last
    unnamed surface in this project: the zarr stores carry `audit_fixes` in
    their attributes and the threshold files are date-stamped, but
    cat_outputs/ was not, and that is the one that bit.
    """
    p = outputs / f"per_diagnostic_{season}_{severity}{tag}.csv"
    if not p.exists():
        return None
    with p.open() as fh:
...
def main() -> int:
    ...
    ap.add_argument("--tag", default="",
                    help="suffix selecting which per-diagnostic series to "
                         "score: '' (default) = the baseline, '_audit' = the "
                         "audit re-derive. The scorecard's own outputs take "
                         "the same suffix, so the two never overwrite.")
    ...
    for sev_key, sev_label, ens_fig, diag_fig in [
        ...
        recs = read_per_diagnostic(outputs, "annual", sev_key, args.tag)
    ...
    # ------------------------------------------------------------- write out
    csv_path = outputs / f"scorecard{args.tag}.csv"
    ...
    md_path = outputs / f"SCORECARD{args.tag}.md"
